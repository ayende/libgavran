...
static result_t txn_encrypt_page(txn_t *tx, uint64_t page_num,
  ...
  if (crypto_kdf_derive_from_key(
          subkey, crypto_aead_aes256gcm_KEYBYTES, page_num, TxnKeyCtx,
  ...
  if (sodium_is_zero(metadata->aes_gcm.nonce,
    ...
    // <2>
    randombytes_buf(metadata->aes_gcm.nonce,
                    crypto_aead_aes256gcm_NPUBBYTES);
  } else {
    // <3>
    sodium_increment(metadata->aes_gcm.nonce,
                     crypto_aead_aes256gcm_NPUBBYTES);
  }
...
static result_t tx_finalize_page(txn_t *tx, page_t *page,
  ...
  if (tx->state->db->options.encrypted) {
    ...
    return txn_encrypt_page(
        tx, page->page_num, page->address,
        size_to_pages(metadata->overflow_size) * PAGE_SIZE, metadata);
  } else {
...
static result_t txn_finalize_modified_pages(txn_t *tx) {
  ...
  page_t *modified_pages =
      calloc(state->pages->modified_pages, sizeof(page_t));
  ensure(modified_pages,
         msg("Unable to allocate buffer for modified pages"));
  ...
  for (size_t i = 0; i < modified_pages_idx; i++) {
    ...
    if ((modified_pages[i].page_num & PAGES_IN_METADATA_MASK) ==
      ...
      // we handle metadata page separately, metadata pages *must* be
      // modified
      continue;
...
result_t txn_commit(txn_t *tx) {
  ...
  if (!tx->state->pages->modified_pages) return success();
...
static result_t txn_merge_unique_pages(txn_state_t *state) {
  ...
  while (prev) {
    ...
    while (hash_get_next(prev->pages, &iter_state, &entry)) {
      ...
      if (hash_lookup(state->pages, &check)) continue;

      ensure(hash_put_new(&state->pages, entry));
      entry->address =
          0;  // we moved ownership, detach from original tx
    }
    prev = prev->prev_tx;
  }
...
static void txn_try_reset_tx_chain(db_state_t *db,
                                   txn_state_t *state) {
  if (state != db->last_write_tx) return;

  // this is the last transaction, need to reset the tx chain to
  // default
...
result_t txn_close(txn_t *tx) {
  txn_state_t *state = tx->state;
  tx->state = 0;
  if (!state) return success();  // probably double close?
  if (state->global_state.header.last_tx_id ==
      state->db->active_write_tx) {
  ...
  // <4>
  if (!state->db->transactions_to_free &&
      state != state->db->default_read_tx)
    state->db->transactions_to_free = state;

  // <5>
  if (--state->usages == 0) ensure(txn_gc_tx(state));
...
result_t txn_create(db_t *db, uint32_t flags, txn_t *tx) {
  ...
  if (db->state->options.encrypted ||
      db->state->options.avoid_mmap_io) {
  ...
  // <4>
         ...
         msg("There can only be a single write transaction at a "
             "time, but "
             "asked for a write transaction when one is already "
             "opened"));
...
static result_t get_metadata_entry(uint64_t page_num,
  ...
  page_metadata_t *entries = metadata_page->address;
         ...
         msg("Attempted to get metadata page, but wasn't marked as "
             "metadata"),
         with(metadata_page->page_num, "%lu"),
         with(entries->type, "%x"), with(page_num, "%lu"));
...
result_t txn_validate_page_hash(
  ...
  char hash_base64[sodium_base64_ENCODED_LEN(
  ...
  sodium_bin2base64(
      ...
      hash, crypto_generichash_BYTES, sodium_base64_VARIANT_ORIGINAL);
  sodium_bin2base64(
...
static result_t txn_ensure_page_is_valid(txn_t *tx, page_t *page) {
  ...
  // <2>
  if (tx->state->db->options.page_validation ==
      page_validation_once) {
...
static result_t txn_decrypt_page(txn_t *tx, page_t *page) {
  ...
  if ((page->page_num & PAGES_IN_METADATA_MASK) == page->page_num) {
        ...
        txn_decrypt(&tx->state->db->options,
                    page->address + PAGE_METADATA_CRYPTO_HEADER_SIZE,
                    PAGE_SIZE - PAGE_METADATA_CRYPTO_HEADER_SIZE,
                    buffer + PAGE_METADATA_CRYPTO_HEADER_SIZE,
...
result_t txn_get_page(txn_t *tx, page_t *page) {
  ...
  if (!page->address) ensure(pages_get(tx, page));
...
result_t txn_modify_page(txn_t *tx, page_t *page) {
  errors_assert_empty();
      ...
      tx->state->global_state.header.number_of_pages > page->page_num,
  ...
  if (!original.overflow_size) original.overflow_size = PAGE_SIZE;
...
static result_t txn_add_action(struct cleanup_act **head,
  ...
  struct cleanup_act *cur = calloc(
      1, sizeof(struct cleanup_act) + sizeof(struct mmap_args));
  if (!cur) {
    failed(ENOMEM,
           msg("Unable to allocate memory to register txn action"));
  }
...
result_t txn_register_on_forget(txn_state_t *tx,
                                ...
                                void *state_to_copy,
                                size_t size_of_state) {
  return txn_add_action(&tx->on_forget, action, state_to_copy,
                        size_of_state);
}
result_t txn_register_on_rollback(txn_state_t *tx,
                                  ...
                                  void *state_to_copy,
                                  size_t size_of_state) {
  return txn_add_action(&tx->on_rollback, action, state_to_copy,
                        size_of_state);
}
// end::txn_add_action[]
