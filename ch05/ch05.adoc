...
[source]
.Listing 5.1 - db.h - The file header contains metadata about the file
----
include::./code/db.h[tags=file_header]
----

The header in Listing 5.1 has a magic field, which must be set to the `FILE_HEADER_MAGIC` value. This is how we'll test if a file is a valid Gavran file or
...
Listing 5.2 shows the implementation of `handle_newly_opened_database` and how we handle opening a database. We also need to add a new field to the 
`db_state_t`, to include an in memory copy of the file header. 

[source]
.Listing 5.2 - db.c - Opening a database
----
...
can see how this is done in Listing 5.3.
...
[source]
.Listing 5.3 - db.c - Setting the file header
----
include::./code/db.c[tags=set_file_header]
----

The code in Listing 5.3 has an interesting property. It is using a _transaction_ to modify the data. This is strange, because transactions right now
...
Listing 5.3 is the also first time that _Gavran_ is actually starting to use its own API to implement real functionality. As part of starting up the database
...
Simple, at least as a concept. Let's see what will happen when we sit down to actually implement this... the full details are in Listing 5.4.

[source]
.Listing 5.4 - db.c - Setting up the free space bitmap
----
...
of code to handle this. We also defined several utility functions in Listing 5.4, but there is nothing really interesting in them. 
...
architecture makes me very happy. Let's look at Listing 5.5 to see how this is done.

[source]
.Listing 5.5 - txn.c - Freeing a page in the transaction
----
...
The `free_page` in Listing 5.5 is getting the page that we want to free and zero it as its first action. I'm doing that for the following reasons:
...
of values. We start from the function declaration in Listing 5.6.

[source]
.Listing 5.6 - impl.h - Declaration to find free space in bitmap for allocation
----
...
Listing 5.7 shows how that function works. There are quite a few of moving pieces here, but that piece of code went through several iterations of refactoring
...
[source]
.Listing 5.7 - bitmap.c - Finding a range of free bits in a single word
----
...
`search_bitmap` to search the entire bitmap. THe code for that function in in Listing 5.8

[source]
.Listing 5.8 - bitmap.c - Finding a range of free bits in the entire bitmap
----
...
the right number of pages free, we don't have enough _consecutive_ pages to answer the allocation. We can do better. The key to that is shown in Listing 5.9.
 For such a short function there is a _lot_ of functionality there.

[source]
.Listing 5.9 - bitmap.c - Finding the _best_ free range for an allocation
----
...
We can now look at `search_free_range_in_bitmap` and see how it works, take a look at Listing 5.10.

[source]
.Listing 5.10 - bitmap.c - The implementation for `search_free_range_in_bitmap` to allocate bitmaps
----
include::./code/bitmap.c[tags=search_for_smallest_nearby]
----
