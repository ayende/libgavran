...
3. Read tx reads page 2, see no changes (the data is held in buffer private to the write tx)
4. Write tx commits & closed
5. Read tx reads page 2,, see the changes that happened because `txn_commit` wrote it.

That is the current behavior of Gavran, and it is almost where we want it to be. The only change in behavior we need is that when the write transaction
...
In fact, there is very little that is going there at all. In `txn_commit`, we are updating a value at the _db_ level, the `last_write_tx`. Let's us see what 
this is used for and see the changes for `txn_create` in Listing 7.2.
