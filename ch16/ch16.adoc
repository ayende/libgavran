...
a bare metal server with 24 TB of RAM. This has 8 cores, so you have separate NUMA nodes to take into account. It has 448 cores running `Intel Xeon 8280L`. These chips has
...
You can see that the API is very similar to the one we have built for hash tables. Unlike hash tables, with just `hash_val_t`, B+Tree we'll `btree_val_t` for reading,
writing and deleting items from tree and `btree_cursor_t` to forward and backward iteration on the tree. That can happen from the beginning or end of the tree or from
any given point. 
...
Conceptually, the idea of a B+Tree is fairly simple and obvious, but a key issue with 
****
