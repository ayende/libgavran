...
for additional uses. That is specifically for this purpose. The `page_metadata_t` has `crypto` as its first field. You can see what it looks like in <<page_crypto_metadata_t>>.
...
pages, we had the entire page (or pages). But for metadata pages, we skip the first 32 bytes of the page. That is because we are actually storing the metadata page has on the first 32 bytes of the
...
fine and working. <<wal_validate_recovered_pages>> shows how we do the actual validation. We register all the modified pages in the `modified_pages` hash table, which allows us to then scan through 
...
----
include::./code/10_wal.c[tags=wal_validate_recovered_pages]
----
...
The actual validation function is `txn_ensure_page_is_valid()`, which is called from `txn_get_page()`. <<txn_ensure_page_is_valid>> shows how it is implemented.
...
The actual validation is done in `txn_validate_page()`, shown in <<txn_validate_page>>.
...
* The has doesn't match, but the page is zeroed and the expected hash is zero. In this case, we consider this to be okay as well. We are looking at an uninitialized page.
...
See the `encryption_key` field on the `db_options_t` struct in <<page_crypto_metadata_t>>. That key isn't actually used for any sort of encryption, instead, we use that key to _derive_
...
We initialize this table in `txn_create()` and we clear it in `txn_close`. I'm showing just the relevant portion of `txn_close()` in <<working_set_txn_close>>, to show how we remove the decrypted
data from memory.
...
Note that `txn_raw_modify_page()` or `txn_get_metadata()` aren't touched, they are already implemented in terms of `txn_raw_get_page()`, so everything just works. 
...
The alternative to `XChaCha20-Poly1305` was to use `AES-GCM`, it also has hardware support (in fact, it is very 
...
It would be easy to use different encryption algorithm on different platforms, but then we have to deal with a database that can be opened on one machine and not on others. As you can 
tell, the actual algorithm we use is going to be very easy to change if needed and I'll wait for benchmark results to see if any changes are required.
****
...
----
include::./code/test.c[tags=crypto_hash]
----
