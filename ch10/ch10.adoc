...
[source]
.Listing 10.10 - db.c - Setting up the state of the database after the file size has been increased
----
...
[source]
.Listing 10.11 - db.c - Moving the free space bitmap when we run out of room for it in its old location
----
...
As you can see, we have clear separation of responsibilities here. The abstraction shown in Listing 10.12 allows me to write the recovery logic
without needing to care about where or how I get the data. That means that we now need to consider how exactly we'll manage the WAL beyond a 
single file.
...
Instead, Gavran takes the position that the user can run a `ln -s /db/phones.wal /mnt/fast/wals/phones.wal` and everything will work as expected
...
I would much rather have a single file, but we can't have that and allow for continuously opened read transactions, as we saw in Figure 20. Creating
...
clear the one that isn't being used when there are active transactions and then switch to it, and so on. You can see how that would work in Figure 21.

.Two WAL files used to store the transaction history of a database
image::{img-src}/fig21.png[]

Figure 21 shows the two files and the transactions that are stored in them. Let's consider the recovery process from such a state.
...
In the case of Figure 21, we'll have the following steps:

* Compare tx 14 from the `B` file to tx 17 from `A` file and decide to use `B`.
...
Now, we can do a checkpoint when the oldest active transaction is beyond the oldest WAL file. We'll `fsync` the data file and then write a valid empty
transaction at the beginning of the oldest WAL file. That ensures that we'll no need to apply any of the data in the WAL file. We'll still need to 
...
of transactions data, we won't need to change the `wal_recover` function. We'll start with looking into the `wal_recovery_operation` and its associated
functions. Listing 10.14 shows the changes to the structures.

[source]
.Listing 10.14 - impl.h & wal.c - Changes to WAL structures to reflect two WAL files
----
// from impl.h
include::./code/impl.h[tags=wal_state_t]

// from wal.c
include::./code/wal.c[tags=wal_recovery_operation]
----
<1> The last transaction id that was written to this WAL file.
<2> The current WAL file index that will be written to by the next `wal_append` operation.
<3> Array to hold the two WAL file states. 
<4> References to the WAL file states, these are *not* necessarily in the same order as the value in the `wal_state_t`. They are sorted on recovery
by the earliest transaction in the file.
<5> The current WAL file that we are _recovering_ from, *not* the same as the `current_append_file_index`. 

The `struct wal_recovery_operation` is used solely for the purpose of recovery, while the `wal_state_t` and `struct wal_file_state` are used mostly
for the purpose of appending to the WAL.
Note how we have an array of two files in each of the structures. For the `wal_state_t`, the order is that the `A` file is first and `B` file is second.
For the `struct wal_recovery_operation`, the order is dependant on the value of the earliest transaction recorded. Listing 10.15 shows how we initialize
`struct wal_recovery_operation`. 

[source]
.Listing 10.15 - wal.c - Initialize the state of the recovery process
----
...
that there are no valid transactions in the file or a valid transaction with no pages, which is the indicator that the WAL file was reset.
...
The code for that is shown in Listing 10.16.

[source]
.Listing 10.16 - wal.c - Finding the next valid transaction (potentially skipping files)
----
include::./code/wal.c[tags=wal_next_valid_transaction]
----
<1> Check for end of file or if we have a non valid transaction.
<2> We now need to check to the end of the file, to see if there are valid transactions after an invalid one (meaning, disk corruption). This used
to run in `wal_recover`, but we now run it here, because it needs to run on _both_ files.
<3> If we are at t second file, we are done and can stop the iteration, there are no more possible transactions.
<4> There is still a file that we need to run through, let's setup the required fields to start iterating there.
<5> Recursively call ourselves so we'll do validation on the next file transaction, etc.
<6> Extract the transaction, if required, and advance to the next transaction for the next call.
...
If we find a valid (but old) transaction, we can skip its whole size to the next possible location to scan. If we find a valid transaction that has a
transaction id that is larger than the highest valid transaction id we have, we know that there is a problem. At this point, we run into something
that broke the chain, there is a missing transaction that we can't apply. That is a cause for alarm and will abort the recovery process with an error.
The code for doing so is shown in Listing 10.17.

[source]
.Listing 10.17 - wal.c - Searching the WAL file for valid transactions after invalid data (indicating disk corruption)
----
include::./code/wal.c[tags=wal_validate_after_end_of_transactions]
----
<1> Get the next range to scan for transactions.
<2> Check if the transaction is valid (verified using the hash we computed).
<3> Move forward by one page (we cannot trust the data, so we scan one page at at time).
<4> Check if the valid transaction is older than the last valid one.
<5> Move to the next page following the transaction size (we trust the data, it passed the hash test, so it is not corrupted or partial).
<6> Report the error to the caller and abort the recovery process.
...
The `wal_validate_after_end_of_transactions` uses a few utility functions that are shown in Listing 10.18, for completion's sake.

[source]
.Listing 10.18 - wal.c - Abstracting the range access across files and managing the freeing of the buffer used during recovery.
----
include::./code/wal.c[tags=wal_range;free_wal_recovery]
----
...
This is shown in Listing 10.19.

[source]
.Listing 10.19 - wal.c - Completing the recovery process by updating the in memory state of the database.
----
...
The `(cur_file_index + 1) & 1` in Listing 10.20 is used to refer to the _other_ WAL file, regardless of what the current one it. This basically
implements overflow addition in the range [0,1]. 

[source]
.Listing 10.20 - wal.c - Checkpoint implementation using double buffering between the files
----
include::./code/wal.c[tags=wal_checkpoint]
----

The `wal_will_checkpoint` will return `true` under the following conditions. The current WAL file is over half full and the other WAL file 
latest transaction is old enough to ensure that no one is looking at the data from it (meaning we wrote it to disk). Note that this may
also trigger for the _current_ file, if there haven't been enough transactions to fill both WAL files before a checkpoint can occur. 

The `wal_reset_file` will overwrite the first page of the WAL with an empty marker transaction, ensuring that we won't consider that to be 
a valid transaction and will discard the whole file. And in `wal_checkpoint` we simply reset the other file and see if we can reset the 
...
behavior in Listing 10.20 will make sure that we have just a single WAL file in use. Why bother?
...
If the WAL file is too large, we'll need to truncate it, we do that using the `palfs_truncate_file` call, shown in Listing 10.21.

[source]
.Listing 10.21 - fs.c - Truncating a file
----
include::./code/fs.c[tags=palfs_truncate_file]
----

I'm showing the function here as a reminder of the complexity we have to deal with when we are working with modifying the file metadata. We
also have to `fsync` the _parent_ directory to make sure that the change "take".

===== Appending to the WAL

Appending to the WAL using dual WAL files didn't change all that much. We just need to be careful not to exceed the size of the file, I moved
...
us to focus solely on what `wal_append` is doing, shown in Listing 10.22. 

[source]
.Listing 10.22 - wal.c - Appending a transaction to the current WAL file
----
include::./code/wal.c[tags=wal_append]
----

The real work if now done in `wal_prepare_txn_buffer` and `wal_append` simply check if there is space in the current WAL file and increase the
...
buffering for the WAL file to handle continuously running transactions. There has been a _lot_ of changes in the code, but I think we are almost 
done working at this layer of Gavran. We have one more chapter to go through, and then we can move on to actually using Gavran to store real data.
