...
Having a single file for the WAL is useful, but it also have some important properties. We don't keep messing with the file system, by allocating
...
of transactions data, we won't need to change the `wal_recover` function.
