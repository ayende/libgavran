...
of the iteration in the page between invocations of `hash_page_get_next()`. The `iter_state` holds the position of the _next_ byte that we need to read. We make sure that it
will always skip the header byte and we run through all the bytes used by the bucket. We continue to do so until we run out of buckets in the page. 
