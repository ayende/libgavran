...
.The `flags` field
====
The `hash_val_t` has three fields that are of particular interest to us. The `key`, `val` and `flags` fields are all set by the user and persisted to the table. In other words,
the actual model for the hash table is `map<uint64_t key, { uint64_t val, uint8_t flags}>`.

The hash table simply persists the `flags` and `val` values. The idea is that callers can use the `flags` to provide some additional meaning about the _type_ of value that is 
stored in the hash table. We'll see how that is used in Chapter 17.
====
...
which is used to remember the iteration position between calls to the `hash_get_next()` function.
...
we aren't going to store the entry data as is. We are going to use `varint` encoding for this purpose. There are _many_ formats for https://github.com/stoklund/varint[varints]. 
The one that we are going to use was taken from the https://github.com/danburkert/bytekey/[bytekey repository]. 

The `bytekey` format uses a 4 bit prefix to denote the length of the key, unlike the common 7 bit varint encoding such as the one used by 
https://developers.google.com/protocol-buffers/docs/encoding[Protocol Buffers]. Let's compare the two options in terms of the size that they take, you can see the full details
in <<bytekey_vs_varint>>.

[[bytekey_vs_varint]]
.Encoded size per numeric range for `bytekey` and `7 bits varint` 
|===
| bytes | `bytekey`                                                 | `7 bits varint`
|  1    | 0 .. 15                                                   | 0 .. 127
|  2    | 16 .. 4095                                                | 128 .. 16,383
|  3    | 4096 .. 1,048,575                                         | 16,384 .. 2,097,151
|  4    | 1,048,576 .. 268,435,455                                  | 2,097,152 .. 268,435,456
|  5    | 268,435,456 .. 68,719,476,735                             | 268,435,456 .. 34,359,738,367
|  6    | 68,719,476,736 .. 17,592,186,044,415                      | 34,359,738,368 .. 4,398,046,511,103
|  7    | 17,592,186,044,416 .. 4,503,599,627,370,495               | 4,398,046,511,104 .. 562,949,953,421,311
|  8    | 4,503,599,627,370,496 .. 1,152,921,504,606,846,975        | 562,949,953,421,312 .. 72,057,594,037,927,935
|  9    | 1,152,921,504,606,846,976 .. 18,446,744,073,709,551,616   | 72,057,594,037,927,936 .. 9,223,372,036,854,775,807
|  10   | N/A                                                       | 9,223,372,036,854,775,808 .. 18,446,744,073,709,551,616
|===

The maximum size of `bytekey` is 9 bytes, while `varint` is maxed at 10 bytes. However, for smaller values, the `varint` tends to be a bit cheaper. So why chose the `varint` 
encoding?

The idea is that we can take a `uint64_t` value and encode it to a buffer. Both formats achieve this goal, but there is a very important distinction between them. If we compare
the buffers of two values generated with `varint` using `memcmp()`, we'll not get a result based on the values. This is because the `varint` output is not going to be ordered
in the same manner as the values that it encodes.

The `bytekey` format, on the other hand, allows us to directly compare the buffers using `memcmp()` and get result (less than, equals or greater than) that would match the actual
encoded value. This doesn't matter too much right now, but it will become much more important when we start working with B+Tree, where the sort order of the key _matters_. 
For that reason, I chose this format for Gavran. 
...
to store an `uint64_t` we encode the data so it will take between 1 - 9 bytes. The size of the encoded `varint` depends on the value we encode. The bigger the value, the more bytes
we need to encode the number. Note that we'll only cross the 8 bytes threshold if the value is larger than `1,152,921,504,606,846,975` so up until that point, this is an absolute win.
...
of the iteration in the page between invocations of `hash_page_get_next()`. The `iter_state` holds the index of the page in the directory and well as the position of the _next_
byte that we need to read. We continue to do so until we run out of buckets in the page. 

In <<hash_get_next>>, where we iterate over the whole hash table, you can see that we set the `iter_state.pos_in_page` to zero when we move between pages. That ensure that 
we scan through each page from the start all the way to the end.
