...
[source]
[[pal-interface]]
.pal.h - The file system interface that we'll consume for our storage engine
----
include::./code/pal.h[tags=pal_api]
----
...
With `verify()` also validating the `errors_count()`, we know that even if the error happened in the `defer()` call, we'll detect and treat it just the same.
====
...
.<<pal_ensure_full_path>> - pal.linux.c - Ensuring that the full path provide exists and the caller has access to it
----
...
.pal.h - Flags for opening a file
----
include::./code/pal.h[tags=pal_file_creation_flags]
----
...
[source,c]
[[read_write_io]]
.Using the file API to read & write
----
...
In this case, you already saw the test cases, <<read_write_io>> and <<create_and_set_file>>, so I won't bother repeating them. 
