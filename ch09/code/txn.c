...
static bool lookup_entry_in_tx(txn_state_t *state, page_t *page) {
  ...
  if (!number_of_buckets) return false;
...
result_t txn_commit(txn_t *tx) {
  ...
  if (!tx->state->modified_pages) return success();

  ensure(txn_raw_modify_page());
...
result_t txn_write_state_to_disk(txn_state_t *state,
  ...
  if (can_checkpoint &&
      wal_will_checkpoint(state->db, state->tx_id)) {
...
static result_t txn_merge_unique_pages(txn_state_t *state) {
  ...
  while (prev) {
    ...
    for (size_t i = 0; i < number_of_buckets; i++) {
      ...
      if (lookup_entry_in_tx(state, &check)) continue;
