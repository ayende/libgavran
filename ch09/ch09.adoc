...
buffer outside of this function. To enable that scenario, we use the `address` field in the `reusable_buffer_t` we are passed. This allows us to keep the buffer
...
I've added `defer(free, recovery_state.tmp_buffer.address);` to the `wal_recover()` function to handle the releasing of this buffer. 
