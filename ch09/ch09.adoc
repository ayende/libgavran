...
Take a look at Listing 9.10 and the call to `txn_write_state_to_disk`, immediately before that we set the `tx_id` to its max value to prevent
...
Note that we aren't calling `txn_modify_page` but to `txn_modify_page_raw`. That function is shown in Listing 9.11.

[source]
.Listing 9.11 - txn.c - Getting a raw page from the data file
----
...
[source,python]
.Listing 9.13 - Unit transaction verification and WAL compression
----
