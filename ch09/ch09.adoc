...
as a checkpoint, clearing the WAL when we know that old transactions are safely stored in the data file. The implementation we have in the previous chapter
...
cheap consumer disks that don't implement `fsync()` or FUA (Flush Unit Access, what `O_DIRECT | O_DSYNC` is translated into) properly.
****
...
we can skip the whole range of the transaction. A valid transaction that is older than the most recent transaction means that the WAL was reset, and we can skip
checking the rest of the file. That is a concern because we want to reduce the startup time as much as possible.
...
If we _didn't_ find such a transaction, we'll move to the next page and the cycle repeats to the end of the WAL. The point in `wal_validate_after_end_of_transaction()`
is that we'll run through the WAL and any corruption will cause us to skip the transaction. We'll then scan for more valid transactions and if there are any, we can
detect that there was an error. Note that a corruption on the _last_ transaction isn't something that we can detect. There is no way to separate that from a transaction
that wasn't committed properly to the disk and we'll have to consider that as rolled back.
...
```
include::./code/08_txn.c[tags=txn_commit]
```
...
The `previous` field in `page_t` is set  `original.address` in `txn_raw_modify_page()` for this exact purpose.
...
The code in <<wal_apply_diff> is _really_ simple. Run over the data set and either zero the memory or copy from the buffer. I'm really proud of how simple the code ended 
...
====
The diff format Gavran uses has almost no requirements from the underlying system. As long as we apply the diffs in order, we can start from any state of the page from the
...
7. `[7,0,7,0]` - the diff is `7 . . 7`
8. `[7,8,7,8]` - the diff is `. 8 . 8`
9. `[9,8,7,8]` - The diff is `9 . . .`
10. `[9,10,10,8]` - The diff is `. 10 10 .`
...
be: `[7, 8, 7, 8]`. Note that this is an _invalid_ result. There have been no transaction that resulted in this value in the page. If we'll run the entire WAL on the page,
...
because it memory allocation and de-allocations are a common cause for performance issues. I'm going to write the simplest code that I can write right now and
...
need to recover from the transaction. This is done in the `wal_validate_transaction()` which we looked at earlier in this chapter using the
