...
That is a _lot_ of state to manage, and it makes it easy to make mistake. Listing 1.1 shows a function that has to deal with coordination across merely two
...
are implemented in Listing 1.2.
...
<5> Helper macros to add `defer` support for other functions. We saw it being used in Listing 1.1. The `enable_semicolon_after_macro_` piece is there to allow
...
The `\_errors_messages_codes`, in turn, is a parallel array of the actual error codes that we got. This memory layout is shown in Figure 1.

.The actual error messages are stored in a thread local buffer that is pre-allocated
image::{img-src}/img1-1.png[]
...
[source]
.Listing 1.5 - errors.c - String processing routines
----
