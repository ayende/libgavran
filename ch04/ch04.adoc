...
the modified pages. This is initialized by the `pagesmap_new()` call. You'll note that we are passing a `flags` parameter to the `txn_create()`, but not doing much of it. I'm currently 
...
The most important piece in `txn_raw_modify_page()` is the management of the `modified_pages` on the transaction. This is a pointer to `pages_map_t`, which is a hash table for 
...
The good news here is that this hash table only need to support `pagesmap_put_new` and `pagesmap_lookup`, we don't need to handle deletions at all. That makes the overall complexity much easier.
...
`Dictionary<K,V>` is a prime example of that. You have the container data and the data held by the container and they can live very far apart. That is not the case here, we always keep them
 very close by. You can see how the `pages_map_t` is defined in <<pages_map_t>>

[source]
[[pages_map_t]]
.`gavran/internal.h` - The hash table definition
----
include::../include/gavran/internal.h[tags=pages_map_t]
----
...
of reference when doing the search, the entire hash table is typically a single allocation and the `pagesmap_lookup` and `pagesmap_put_new` implementations
are straightforward.
...
In <<pagesmap_new_and_lookup>> you can see how we create a new hash table using `pagesmap_new()` and looking up a single value using `pagesmap_lookup()`. The
transaction's `modified_pages` field is setup via `pagesmap_new()` in `txn_create()`. We are going to be using this hash table a _lot_. It is going 
...
[source]
[[pagesmap_new_and_lookup]]
.`hashtable.h` - Creating a hash table and looking up a single value
----
include::./code/hashtable.c[tags=pagesmap_new_and_lookup]
----

In <<pagesmap_new_and_lookup>> you can see that creating a new hash table is simple, we allocate a single buffer, for the `pages_map_t` as well
as the `entries` immediately following it. That is why we have an "empty" array at the end of `pages_map_t`, that indicates to the compiler
...
This is the more complex operation because we may need to resize the hash table if the load factor grows big enough. The details are in <<pagesmap_put_new>>.

<<pagesmap_put_new>> starts by expanding the hash table if needed, then scans through the table for an empty location that we can place the new value in,
...
check if the _next_ call to `pagesmap_put_new()` should expand the table. This is done by checking if we are over 75% full. As mentioned, I'm not going to 
...
----
include::./code/hashtable.c[tags=pagesmap_put_new]
----
...
<<pagesmap_expand_table>> shows how we double the size of the table. We allocate a new buffer, twice as large, iterate over the old hash table using `pagesmap_get_next()`
and add to the new hash table using `pagesmap_put_new()`, pretty simple, overall.
...
[source]
[[pagesmap_expand_table]]
.`hashtable.h` - Doubling the size of the hash table
----
include::./code/hashtable.c[tags=pagesmap_expand_table]
----

There are a few interesting implications to the way we built `pagesmap_expand_table()`. First, any `pagesmap_put_new()` may cause the table to shift, that is why 
we send an indirect pointer to the function. 
You cannot mix `pagesmap_get_next()` and `pagesmap_put_new()`, iteration and mutations generally don't mix with one another.

Finally, let's look at `pagesmap_get_next()`, shown in <<pagesmap_get_next>>.
...
We expect `pagesmap_get_next()` to be used as it is used in <<pagesmap_expand_table>>. Initialize the `iter_state` value to `0` and call `pagesmap_get_next()` until it returns `false`. 
...
----
include::./code/hashtable.c[tags=pagesmap_get_next]
----
