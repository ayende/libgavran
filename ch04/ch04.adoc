...
The `struct database_state` (shown in Listing 3.1) itself isn't very big at the moment. It contains the database options (currently only the minimum size of the database), 
...
.Memory alignment
****
In Listing 3.7 you can see that I'm using `palmem_allocate_pages` function, that make use of `posix_memaligned` under the covers. I'm asking the system
...
The details are in Listing 3.9.
...
[source,python]
.Listing 3.13 - Unit testing the transaction API
----
