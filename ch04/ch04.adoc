...
<<hash_expand_table>> shows how we double the size of the table. We allocate a new buffer, twice as large, iterate over the old hash table using `pagesmap_get_next()`
...
There are a few interesting implications to the way we built `hash_expand_table()`. First, any `pagesmap_put_new()` may cause the table to shift, that is why 
...
We expect `pagesmap_get_next()` to be used as it is used in <<hash_expand_table>>. Initialize the `iter_state` value to `0` and call `pagesmap_get_next()` until it returns `false`. 
