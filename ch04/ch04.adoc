...
note that this is a simplified version of what we'll have in the future. Whenever you see a code sample that come from a `<name>.simplified.c`, you can assume that this is going to 
be reworked on in later chapters. You can see in <<db_validate_options>> that the code validates the provided settings and copy them to our own `db_options_t` struct. 
...
[source]
[[db_validate_options]]
.`db.simplified.c` - Validating the database's options and default options initialization
----
include::./code/db.simplified.c[tags=db_validate_options]

include::./code/db.simplified.c[tags=db_initialize_default_options]
----
...
[source]
[[txn_create]]
.`txn.simplified.c` - Creating a transaction
----
include::./code/txn.simplified.c[tags=txn_create]
----
...
[source]
[[txn_raw_get_page]]
.`txn.simplified.c` - Get a page in the scope of the transaction, either locally modified copy or directly from the data file.
----
include::./code/txn.simplified.c[tags=txn_raw_get_page]
----
...
[source]
[[txn_raw_modify_page]]
.`txn.simplified.c` - Implementing Copy on Write in the transaction for page modifications.
----
include::./code/txn.simplified.c[tags=txn_raw_modify_page]
----
...
[source]
[[txn_commit]]
.`txn.simplified.c` - Implementing commit transaction by writing modified pages to disk and closing it by freeing the memory of the transaction
----
include::./code/txn.simplified.c[tags=txn_commit]

include::./code/txn.simplified.c[tags=txn_close]
----
