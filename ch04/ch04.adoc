...
relevant definitions in Listing 4.1. 

[source]
.Listing 4.1 - db.h & impl.h - The declarations of db_t and txn_t
----
...
In Listing 4.1, we also define the `PAGE_SIZE` constant and the `page_t` struct. There isn't much there at this point, we simply have the page number and
...
usage within our API, so we'll wrap it in `txn_allocate_page`, shown on Listing 4.2.

[source]
.Listing 4.2 - mem.c - Allocating memory in chunks of pages
----
...
[source]
.Listing 4.2 - main.c - Reading and writing using the paging API
----
...
copy of the memory I used and move on without need to write compensation logic to restore things to the way it was. Listing 4.3 shows the implementation of
`pages_get` and `pages_write`.

[source]
.Listing 4.3 - paging.c - Reading and writing using the paging API
----
...
Look at Listing 4.4, which shows the API we are going to expose for our users. 

[source]
.Listing 4.4 - db.h - Transaction API declaration for our storage engine
----
include::./code/db.h[tags=tx_api]
----
<1> Forward declaration of opaque data types holding the state that the engine will need to manage its work.
<2> The publicly defined structs that users will use to hold the data about databases and transactions.
<3> The database options as strongly typed values. Currently very minimal.
<4> Database create and close APIs.
<5> Enabling `defer` support for closing a database.
<6> Transaction create and close APIs.
<7> Enabling `defer` support for closing a transaction.
<8> Page modifications API.

The idea in Listing 4.4 is that we want to do _everything_ within the scope of a transaction. Note that if you want to get or modify a page, you must provide a 
...
This is where we start to put together the skeleton of Gavran. Let's take a look at the first stage, creating a database, shown in Listing 4.5.

[source]
.Listing 4.5 - db.c - Creating an database instance
----
include::./code/db.c[]
----
...
A `db_t` is a simple structure that holds a single opaque pointer. I'm using the same approach in `txn_t` as well and it means that I have encapsulation as well
...
The `struct database_state` (shown in Listing 4.1) itself isn't very big at the moment. It contains the database options (currently only the minimum size of the database), 
the file handle and the memory map result. As times goes by, we'll see more and more data being managed at the database level, but it is important to start with the
proper structure now, to avoid extra work later.
...
You already saw the external API to create a transaction in Listing 4.4, now we want to actually build it. Before we can do that, I want to go over what transactions _are_.
...
Listing 4.6 shows the structure of a transaction as well as its initialization.

[source]
.Listing 4.6 - impl.h & txn.c - Creating a transaction
----
...
`entries` buffer is placed after the `txn_state_t`. Data locality is a huge performance boost and I want some of that.
...
separation between the data containers and the values. 

`Dictionary<K,V>` is a prime example of that. You have the container data and the data
...
[source]
.Listing 4.7 - Modifying a page inside a transaction
----
include::./code/txn.c[tags=txn_modify_page]
----

The `txn_modify_page` function in Listing 4.7 will first check if we already asked to modify the page in this transaction. If so, that copy of 
...
.Memory alignment
****
In Listing 4.7 you can see that I'm using `palmem_allocate_pages` function, that make use of `posix_memaligned` under the covers. I'm asking the system
...
the `lookup_entry_in_tx` in Listing 4.8, where we are searching through the hash table for a match.

[source]
.Listing 4.8 - txn.c -Looking up a page in the transaction's hash table
----
...
Now that we know how to get an entry from the hash table, let's look at the other side, how can we put a value in the table?
This is the more complex operation because we may need to resize the hash table if the load factor grows big enough. 
The details are in Listing 4.9.

[source]
.Listing 4.9 - txn.c - Adding an entry to the hash table, potentially resizing it
----
include::./code/txn.c[tags=allocate_entry_in_tx]
----

The code in Listing 4.9 starts out in the same manner as `lookup_entry_in_tx` does, by doing a scan on the buckets from the starting position based on the
...
The last piece of the puzzle for the hash table is the `expand_hash_table` function, which is covered in Listing 4.10. Inside this function you
can find the reason we use the `state` pointer inside `txn_t`. 

[source]
.Listing 4.10 - txn.c - Expanding the hash table
----
include::./code/txn.c[tags=expand_hash_table]
----

The code in Listing 4.10 allocates a new bucket buffer, double the size of the old one (plus the size of the `txn_state_t`). If we fail this allocation, we let the 
caller know about this and they can decide how to handle it. You saw the two behaviors we have for this error mode in Listing 4.9.
...
Checking Listing 4.12, where we show how to get a page for read only operations.

[source]
.Listing 4.12 - txn.c - Getting a page from the transaction for read only purposes
----
include::./code/txn.c[tags=txn_get_page]
----

This code is built so if you modified the page and then another part got the same page in the same transaction, you'll get the _modified_ page, not the 
original one. That allows us to a unified view on the data inside a transaction.

On the other hand, a sequence of `txn_get_page` (get the original), `txn_modify_page` (copy and create a new one) and then `txn_get_page` is something
that you need to be aware of. The first and last `txn_get_page` will return _different_ values. I haven't found this a real issue in practice, but something
...
.Listing 4.11 - txn.c - Committing the modified pages to disk
----
include::./code/txn.c[tags=txn_commit]
----

There really isn't much to do in Listing 4.11, to be honest. We scan through the table of modified pages and write them to the file using the `palfs_write_file` function
...
After the transaction is committed, we can close it. Alternatively, we can close the transaction without committing it to as a rollback. Listing 4.12 shows the
relevant code.

.Listing 4.12 - txn.c - Closing the transaction and freeing resources
----
include::./code/txn.c[tags=txn_close]
----

Closing the transaction in Listing 4.12 means that we iterate over the modified pages and free them, then we are done. 
...
The last thing that we'll do in this chapter is put everything together. Listing 4.12 shows how we can now write Listing 4.3 using the new API.

[source]
.Listing 4.12 - main.c - Reading and writing using the paging API
----
...
In Listing 4.12, you'll note that we are using _two_ transactions. One to do the writes and one for the reads. We actually got a _very_ limited form
of isolation. Until the `write_tx` is committed, the transaction in `print_msg` will not see any of its changes. I wrote it in this manner to show
how we are starting to get real features. 
The reason the code in Listing 4.12 is able to create isolation between transactions is that the `write_tx` doesn't modify the file directly, it modified a 
...
[source,python]
.Listing 4.13 - Unit testing the transaction API
----
include::../pyapi/tests04.py[]
----

The actual Python API I'm using here is not mirroring the C API directly, mind you. I'm trying to get a more realistic API that wraps some C code. It also
makes for a far simpler system to setup more complex scenarios, which is where we are heading.
