...
def test_can_get_error_recorded():
    Errors.push(b"code.c", 12, b"run", 22, b"Opps")
...
def test_push_multiple_errors():
    Errors.push(b"code.c", 12, b"run", 22, b"Opps")
    Errors.push(b"code.c", 13, b"call", 22, b"um...")
...
def test_max_errors_is_64():
    for i in range(100):
        Errors.push(b"code.c", i, b"run", 22, b"Opps")

    assert Errors.get_count() == 64
    
def test_very_large_errors_wont_overflow():
    for i in range(100):
        Errors.push(b"code.c", i, b"run", 22, b"x" * 256)
