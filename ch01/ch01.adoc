...
<3> The `defer` and `try_defer` allow us to register a function to be called when the current scope ends. This capability uses the `__attribute__(cleanup)` feature
...
<3> The `defer_free` function is a good example of how we implement handling of `try_defer` functions. The captured value will be freed unless the caller has
...
It is actually easy enough to provide a macro that would generate the appropriate deferring function. Consider the following code:

----
#define register_defer(func, type)                                             \
  void defer_##func(struct cancel_defer *cd) {                                 \
    if (cd->cancelled && *cd->cancelled)                                       \
      return;                                                                  \
    type *a = *(type *)cd->target;                                             \
    func(a);                                                                   \
  }

register_defer(free, void*);
----

That (or something similar) would be sufficient. And for `free`, this actually works. After all, `free` can't fail. But closing a file can, and we _need_
to be able to report this information _somewhere_. That is why we require that the user would provide these functions and the associated error handling.
...
compiler to do its magic. The last line of both macros is meant to avoid a warning about unused variable, since I'm compiling will all warnings enabled and we
