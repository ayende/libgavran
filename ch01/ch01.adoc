...
.Gavran meaning
====
Beside meaning raven, Gavran is also a word in Hebrew. It is a term that can be roughtly translated as:

* Very strong
* Powerful 
* Overcoming challanges

I find the name highly amusing on several levels.
====
...
The `_errors_messages_codes`, in turn, is a parallal array of the actual error codes that we got. This memory layout is shown in Figure 1.1.

.Figure 1.1 - The actual error messages are stored in a thread local buffer that is pre-allocated
...
Howard Chu, author of LMDB, discussion https://www.openldap.org/lists/openldap-software/200303/msg00560.html[C string API].
